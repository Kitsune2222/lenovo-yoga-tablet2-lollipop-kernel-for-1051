...
#define BQ27541_SLAVE_ADDR 0x78
...
#define BATT_CHRG_FULL_DES		9300000
...
// 			val->intval = 1;
        val->intval = (max17042_read_reg(chip->client, MAX17042_FullCAP) * MAX17042_CHRG_CONV_FCTR * 100) / BATT_CHRG_FULL_DES;
		break;
	case POWER_SUPPLY_PROP_MODEL_NAME:
...
{
    client->addr = 0x78;
    dev_info(&client->dev, "%s: MAX17042 set address 0x%x\n",
			 __func__, client->addr);
	struct i2c_adapter *adapter = to_i2c_adapter(client->dev.parent);
	struct max17042_chip *chip;
	int ret, i, gpio;
	struct acpi_gpio_info gpio_info;
    dev_info(&client->dev, "%s: MAX17042 probe\n",
			 __func__);
...
// 	}
    ...
    dev_info(&client->dev, "%s: MAX17042 registered\n",
			 __func__);
	return 0;
}
...
{
    printk(KERN_ERR "MAX17047 init call\n");
